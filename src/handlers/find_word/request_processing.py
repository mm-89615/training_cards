...
async def main_find(message: Message, state: FSMContext, request: Request):
    ...
    text, kb = await get_find_word_menu(
        request=request,
...
async def find_word(
    callback: CallbackQuery, callback_data: FindCallback, request: Request
):
    if callback_data.action == "delete":
        if callback_data.dict_type == DictType.admin:
            ...
            await request.user_words.delete_word(
                word_id=callback_data.word_id, tg_id=callback.from_user.id
            )
