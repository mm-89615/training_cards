...
def get_words_buttons(
    ...
    keyboard.row(InlineKeyboardButton(
        text="❌ Выйти",
        callback_data=FindCallback(dict_type=dict_type, level=level, action="exit").pack()
    ))
...
def get_word_actions_kb(
    ...
    keyboard.add(InlineKeyboardButton(
        text="❌ Удалить",
        callback_data=FindCallback(dict_type=dict_type,
                                   level=level + 1,
                                   word_id=word_id,
                                   # action="delete",
                                   find_word=find_word,
...
def exit_kb():
    ...
    keyboard.add(InlineKeyboardButton(
        text="❌ Выйти",
        callback_data=FindCallback(dict_type=DictType.user, level=0, action="exit").pack()))
    return keyboard.as_markup()
