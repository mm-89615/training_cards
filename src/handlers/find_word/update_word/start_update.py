...
async def edit_word(
    ...
    if callback_data.dict_type == DictType.admin:
        ...
        word = await request.user_words.get_word_by_id(
            word_id=callback_data.word_id, tg_id=callback_data.user_id
        )
    await state.update_data(
        word_id=word.id, old_en=word.in_english, old_ru=word.in_russian
    )
    kb = get_word_kb(word.in_english)
    await callback.message.delete()
    await callback.message.answer(
        "Введите новое значение слова на английском языке: ", reply_markup=kb
    )
