...
class WordRequests(BaseRequest):
                ...
                Word.id.not_in(
                    (
                        select(UserWord.word_id).where(
                            UserWord.user_tg_id == tg_id, UserWord.word_id.is_not(None)
                        )
                    )
                )
            )
    ...
    async def get_three_random_words(self, word_id: int):
        ...
        """
        stmt = select(Word).where(Word.id != word_id).order_by(func.random()).limit(3)
        result = await self.session.scalars(stmt)
        return result.all()
    ...
    async def get_four_random_words(self, tg_id: int):
        ...
        stmt2 = select(
            UserWord.word_id, UserWord.in_english, UserWord.in_russian
        ).where(UserWord.word_id.is_(None), UserWord.user_tg_id == tg_id)
        union_stmt = (
            select(union_all(stmt1, stmt2).subquery()).order_by(func.random()).limit(4)
        )
        result = await self.session.execute(union_stmt)
        return result.all()

    async def get_word_by_english(self, in_english: str):
        stmt = select(Word).where(Word.in_english.ilike("%" + in_english + "%"))
        result = await self.session.scalars(stmt)
        return result.all()
