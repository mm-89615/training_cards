...
class UserWordRequests(BaseRequest):
    async def add_user_word(
        self, user_id: int, word_id: Optional[int], in_russian: str, in_english: str
    ):
        if word_id:
    ...
    async def get_new_word_from_user_words(self, tg_id: int):
        ...
        interval_30_min = (
            UserWord.updated_at + func.cast(concat(30, "MINUTE"), INTERVAL) < func.now()
        )
        interval_1_day = (
            UserWord.updated_at + func.cast(concat(1, "DAY"), INTERVAL) < func.now()
        )
        interval_1_week = (
            UserWord.updated_at + func.cast(concat(1, "WEEK"), INTERVAL) < func.now()
        )
        interval_2_week = (
            UserWord.updated_at + func.cast(concat(2, "WEEK"), INTERVAL) < func.now()
        )
        interval_2_month = (
            UserWord.updated_at + func.cast(concat(2, "MONTH"), INTERVAL) < func.now()
        )
        stmt = (
            select(UserWord)
    ...
    async def update_date(self, word_id):
        stmt = (
            update(UserWord).where(UserWord.id == word_id).values(updated_at=func.now())
        )
        await self.session.execute(stmt)
        await self.session.commit()
    ...
    async def update_word(
        self, word_id: int, in_english: str, in_russian: str, tg_id: int
    ):
        stmt = (
            update(UserWord)
            .where(UserWord.id == word_id)
            .where(UserWord.user_tg_id == tg_id)
            .values(in_english=in_english, in_russian=in_russian)
        )
        await self.session.execute(stmt)
        await self.session.commit()
